...
- [x] Requires minimal server infrastructure:
  - (required) A [signalhub server]() via instances can initiate their peer-to-peer connection.
  - (optional) A [metadata providing server]() that delivers cover art and information about similarity of songs.
...
- [x] Different sharing levels are avaliable: `leech`, `private` and `everyone`.
  ...
  - `everyone` means that [WebTorrent]() will be used:
